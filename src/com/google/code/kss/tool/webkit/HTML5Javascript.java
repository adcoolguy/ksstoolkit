...
public class HTML5Javascript implements Converter {
...
//	private String stringConcat = " + ";
	...
	private static final int CHROME_X_OFFSET = 8;
	private static final int CHROME_Y_OFFSET = 8;
	private static final int JS_ELEMENT_OFFSET = -1;	//JavaScript element starts with zero not 1
	private String fiosIrisHome;
	...
	public HTML5Javascript(ConverterBase c) {
		super();
		this.c = c;
	}
	...
	public void setNativeSDKHome(String fiosIrisHome) {
		this.fiosIrisHome = fiosIrisHome;
	}
...
//zero element - does not work well (not a supported use case)
//		finalJSCodes.append("var " + listTextName + "\n");
		//zero element
		//finalJSCodes.append("var " + listTextName + "= []\n");
//backward allowed fired to false too early at element 1, also when loaded, clicking foward might have index/flag inverse issue
//		finalJSCodes.append("var " + listTextName + "= ['Real FiOS Text1','Real FiOS Text2','Real FiOS Text3','Real FiOS Text4']" + lineDelimiter + "\n");
//backward allowed fired to false too early at element 1, also when loaded, clicking foward might have init text issue
//		finalJSCodes.append("var " + listTextName + "= ['Real FiOS Text1','Real FiOS Text2','Real FiOS Text3','Real FiOS Text4','Real FiOS Text5']" + lineDelimiter + "\n");
//works perfectly
//		finalJSCodes.append("var " + listTextName + "= ['Real FiOS Text1','Real FiOS Text2','Real FiOS Text3','Real FiOS Text4','Real FiOS Text5','Real FiOS Text6','7 (second to last element)','8 (last element)']" + lineDelimiter + "\n");
		finalJSCodes.append(scriptEndDelimiter + "\n");
		finalJSCodes.append("\n<!-- ------- This Javascript script is generated by KSSTolkit " + releaseNumber
				+ " and you should not have to change anything after this line --------- -->\n");
		finalJSCodes.append("<!-- providing the Java implementation -->\n");
		finalJSCodes.append("<applet id=\"kss1\" codebase=\"./bin\" code=\"com.google.code.kss.core.util.PackageAccess\" name=\"kss\" width=\"0\" height=\"0\"></applet>\n");
		finalJSCodes.append("<applet id=\"app1\" codebase=\"./bin\" code=\"com.google.code.kss.adapter.fios.api.PackageAccess\" name=\"app\" width=\"0\" height=\"0\"></applet>\n");
		finalJSCodes.append("<SCRIPT type=\"text/javascript\" src=\"script/javascript/util/ba-debug.min.js\"></script>\n");
		finalJSCodes.append("<SCRIPT type=\"text/javascript\" src=\"script/javascript/fdt/FDT.js\"></script>\n");
		finalJSCodes.append("<SCRIPT type=\"text/javascript\" src=\"script/javascript/fdt/Graphics.js\"></script>\n");
		finalJSCodes.append(scriptBeginDelimiter + "\n");
		finalJSCodes.append(commentDelimiter + "------- Global variable\n");
		finalJSCodes.append("g_FDT" + componentId + "_Main_Window_Handle = -1" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Text_Window_Handle = -1" + lineDelimiter + "\n");		
		finalJSCodes.append("g_FDT" + componentId + "_Screen_Count = 0" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_First_Screen = 1" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Scroll_Up= " + nullToken + "" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Scroll_Down= " + nullToken + "" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Scroll_Left= " + nullToken + "" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Scroll_Right= " + nullToken + "" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Scroll_XOffset = 0" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Scroll_YOffset = 0" + lineDelimiter + "\n");
		...
		finalJSCodes.append("g_FDT" + componentId + "_step = 0" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Scroll_FWDClippedAtX = -1" + lineDelimiter + "\n");
		finalJSCodes.append("g_FDT" + componentId + "_Scroll_FWDClippedAtY = -1" + lineDelimiter + "\n");
		finalJSCodes.append(scriptEndDelimiter + "\n");
		
		finalJSCodes.append(scriptBeginDelimiter + "\n");
		addTextConstant();
		/* Priority levels - 0 disables all ! The rest:
		log (1) < debug (2) < info (3) < warn (4) < error (5) */
		finalJSCodes.append("debug.setLevel(3)" + "\n");
		finalJSCodes.append("window.onload = function() "+ methodBeginDelimiter  + "\n");
		finalJSCodes.append("mainCanvas  = document.getElementById('FDT" + componentId + "_MAIN_WINDOW_LAYER')" + lineDelimiter + "\n");
		finalJSCodes.append("mainContext = mainCanvas.getContext('2d')" + lineDelimiter + "\n");
		finalJSCodes.append("textCanvas  = document.getElementById('FDT" + componentId + "_TEXT_WINDOW_LAYER')" + lineDelimiter + "\n");
		finalJSCodes.append("textContext = textCanvas.getContext('2d')" + lineDelimiter + "\n");
		finalJSCodes.append("\n");

		finalJSCodes
		.append("\tiAttributeID = Graphics.CreateWindowAttrib()" + lineDelimiter + "\n");
		finalJSCodes
		.append("\tg_FDT" + componentId + "_Main_Window_Handle = Graphics.CreateWindow(FDT" + componentId + "_MAIN_WINDOW_LAYER, iAttributeID, 0, 0, 0, 704, 480)" + lineDelimiter + "\n");
		finalJSCodes
		.append("\tg_FDT" + componentId + "_Text_Window_Handle = Graphics.CreateWindow(FDT" + componentId + "_TEXT_WINDOW_LAYER, iAttributeID, 0, g_FDT" + componentId + "_Scroll_BWDClippedAtX, g_FDT" + componentId + "_Scroll_BWDClippedAtY, g_FDT" + componentId + "_Scroll_FWDClippedAtX, g_FDT" + componentId + "_Scroll_FWDClippedAtY)" + lineDelimiter + "\n");
	}
	...
	private void addImageSrcHandler(StringBuffer finalJSCodes) {
		...
		finalJSCodes.append("var retVal\n");
			...
			if(itr.hasNext()) {
				...
				String temp = "retVal = '" + fiosIrisHome + pathSeparator + "Img" + pathSeparator + "' + lImageID + '.png'";
	...
	public void addFooter(boolean appendFlag, int totalScene, String cmPrefix, Integer[] rectangleOKRegion, String listTextName, boolean useIDEText, ViewType mode, boolean standalone, int textWindowWidth, int textWindowHeight) {
		try {
			...
			canvasHTML.append("<canvas id='FDT" + componentId + "_MAIN_WINDOW_LAYER' style='position: absolute; left: " + (0 + CHROME_X_OFFSET) + "px; top: 0px border: 1px gray dashed' width='704' height='480'>");
			...
			canvasHTML.append("<canvas id='FDT" + componentId + "_TEXT_WINDOW_LAYER' style='position: absolute; left: "+ (mainBgX + CHROME_X_OFFSET) +"px; top: "+ (mainBgY + CHROME_Y_OFFSET) +"px' width='"+ mainBgWidth +"' height='"+ mainBgHeight +"' style='border: 1px gray dashed'>");
			canvasHTML.append("Content that is shown to users using browsers that don't support the canvas tag]");
			canvasHTML.append("</canvas>\n");
